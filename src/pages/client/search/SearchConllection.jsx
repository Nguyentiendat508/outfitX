...
function SearchConllection() {
  ...
  const getSearch = useMemo(() => {
    return products.filter((item) => item.id_cate_type === id);
  }, [id, categoryTypes, products]);
  return (
    <div className="mx-[10%] pb-10">

      <h1 className="text-black my-3 font-semibold  ml-11">
        <Link to={"/"}>Trang Chủ / </Link>{" "}
        {getOjectById(categoryTypes, id)?.name}
      </h1>
      <SlideShowCoverflow data={getSearch} />
      ...
      <div className="grid grid-cols-4 gap-4 mt-3 cursor-pointer">
        {getSearch.map((item) => (
